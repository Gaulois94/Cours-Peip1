...
Si Q est sans dimensions, $\alpha = \beta =...=\epsilon=0$
$[Q]=1$
...
\paragraph{2)} $G_V \in [G_exp - \delta G ; G_exp + \delta G]$
...
\begin{align}
	G_ex & = & f(x_{mesure}) + \frac{df}{dx} (x_{mes}) (x-x_{mes}) + ... \\ 
	G_ex - G_{mes} & \simeq & \frac{df}{dx}(x_{mes})(x-x_{mes}) \\
...
Ecriture d'un résultat : $G = (G_exp +- \delta G)$(Unité) ~\\
$G = G_exp $ à $(\frac{\delta x}{G})$ près.
Exemple :
$V_mesuree$ avec $\delta V $
