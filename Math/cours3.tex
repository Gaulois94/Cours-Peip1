...
\paragraph{Definition} si $x \in E$, on dit que E est un voisinage de x si E contient un intervalle ouvert qui contient x. Ceci est équivalent à E voisinage de x si il existe $\delta > 0$  tel que $]x - \delta; x + \delta[ \in E$.
...
\paragraph{Notation} \[\lim_{x \to x_0} f(x) = l\] ou \[f(x) \rightarrow_{x \to x_0} l\]
...
~\\
On appelle $\delta = min\{d_1, d_2\}, ]x_0-\delta; x_0+\delta[ \subset ]x_0 - \delta_1; x_0+\delta_1[ \cap ]x-\delta_2;x_0+\delta_2[$
~\\
...
(i) Si f, g, h admettent pour limites respectivs l, m, n en $x_0$ et si f(x) $\in g(x) \leq h(x)$ pour tout x de t, alors$ l \leq m \leq n$
~\\
(ii) Si $f(x) \leq g(x) \leq h(x)$ sur E et si f et h admettent une limite (identique) l en $x_0$, alors g admet en $x_0$ et \[\lim_{x \to x_0} g(x) = l\]

\paragraph{Remarque} ON remplace les inégalité de (i) par $\forall x \in E$, $f(x) < g(x) < h(x)$, on obtient $l \leq m \leq n$

\paragraph{Exemple} $f(x) = |x|$ et $g(x) = 2\dot |x|$ Sur $E \subset R^+, f<g$ mais \[\lim_{x \to 0}f(x) = \lim_{x \to 0} g(x) \]

\paragraph{Exemple} \[\lim_{x \to 0} x \dot \sin(\frac{1}/{x})\] existe ?
